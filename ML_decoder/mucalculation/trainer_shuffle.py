...
def fit_model(X, Y, bs, nb_epoch, model, emd):
    y = Y
    ...
    thresh = get_threshold(model, X, Y, bs=4096, emd=1e-3)

    p = model.predict(X, batch_size=4096)

    p_0 = p[Y==0]
    md = np.sum(p_0>thresh)*1.0/(np.sum(Y==0))
    
    p_1 = p[Y==1]
    fa = np.sum(p_1<thresh)*1.0/(np.sum(Y==1))

    print("false_alarms -> ", fa, "mis_det -> ", md)

    return model, thresh

def get_metrics(model, X_test, Y_test, thresh):
...
def plot_md(fname, p):
    ...
    for snr in SNRlist:
        ...
        for w in windowlist:
            ...
            model, thresh = fit_model(X, A, bs=512, nb_epoch=8, model=model, emd=emd)

            for testcase in range(10000):
                X_test, A_test = gen_codeword(p=p, SNR=snr, N=10000, window=w, h1=h1, h2=h2)
                # X_test, Y, A_test = gen_and_process(p=p, SNR=snr, N=10000, window=w, h1=h1, h2=h2)
                fa, md = get_metrics(model, X_test, A_test, thresh=thresh)
...
if __name__ == "__main__":
	# main()
    plot_md(fname='getmumy.csv', p=0.5)